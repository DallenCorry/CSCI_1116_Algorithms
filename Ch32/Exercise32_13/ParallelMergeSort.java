...
public class ParallelMergeSort {
	...
	public static <E extends Comparable<E>> void parallelMergeSort(E[] list) {
		RecursiveAction mainTask = new SortTaskGeneric(list);
		ForkJoinPool pool = new ForkJoinPool();
		pool.invoke(mainTask);
	}
	
	private static class SortTaskGeneric<E extends Comparable<E>> extends RecursiveAction {
		private final int THRESHOLD = 500;
		private E[] list;
		
		SortTaskGeneric(E[] list){
			this.list = list;
		}
		@Override
		protected void compute() {
			if (list.length < THRESHOLD)
				...
				invokeAll(new SortTaskGeneric(firstHalf), 
					new SortTaskGeneric(secondHalf));
