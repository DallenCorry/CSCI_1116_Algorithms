...
public class UnweightedGraph<V> implements Graph<V> {
	...
	*/
	public List<Integer> getACycle() {
		//x needs to be a vertex and an int?? or parent is just an int
		//v needs to be a vertex?
		/*FIX*/
//		List<Integer> myVertices = new ArrayList<>();
//		for(int i=0; i < vertices.size(); i++ ) {
//			myVertices.add(i);
//		}
		List<V> myVertices = new ArrayList<>();
		for(V v:vertices ) {
			myVertices.add(v);
		}
		...
		while(myVertices.size() > 0) {
			...
			while(!stack.isEmpty()) {
				V x = stack.peek();
				List<Edge> neighborsOfX = neighbors.get(getIndex(x));
				if (neighborsOfX.size() < 1) {
					stack.pop();
				} else {
					for (int i=0; i<neighborsOfX.size(); i++) {
						Edge e = neighborsOfX.get(i);
						if(!visited[e.v]) {
							parents[e.v] = getIndex(x);
							stack.push(getVertex(e.v));
							visited[e.v] = true;
							myVertices.remove(getVertex(e.v));
							neighborsOfX.remove(i);
							break;
						} else if(parents[getIndex(x)] != e.v) {
							ArrayList<Integer> anotherList = new ArrayList<Integer>();
							anotherList.add(e.v);
							while(getIndex(x) != e.v && getIndex(x) != -1) {
								anotherList.add(getIndex(x));
								x = getVertex(parents[getIndex(x)]);
							}
							return anotherList;
						} else {
							neighborsOfX.remove(i);
